...
class EBSSnapshotsFactory:
    ...
    def create_snapshot(self, request, subnet_id, region):
        ...
        cluster_config = SnapshotConfig(
            request.config.getoption("key_path"),
            request.config.getoption("key_name"),
            self.ec2.Subnet(subnet_id).vpc_id,
            subnet_id,
        )
        self.snapshot = self._create_snapshot(region, cluster_config)
        return self.snapshot.id

    def _create_snapshot(self, region, cluster_config):
    ...
    def _create_volume_snapshot(self):
        ...
        while snapshot.state == u"pending":
            time.sleep(10)
    ...
    def _launch_instance(self, ami_id, subnet):
        instance = self.ec2.create_instances(
            ImageId=ami_id,
            # SubnetId=cluster_config.master_subnet_id,
    ...
    def _release_instance(self):
        if self.instance is not None:
            ...
            while self.instance.state["Name"] != u"terminated":
                time.sleep(10)
